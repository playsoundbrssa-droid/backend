    ...
    return (
        <section id="features" className="py-20 px-6">
            <div className="container mx-auto max-w-6xl space-y-20">
                ...
                ═══════════════════════════════════════════ */}
                <div className="relative reveal px-2 md:px-0">
                    {/* Glow de fundo */}
                    ...
                    <div className="relative glass-panel rounded-[2.5rem] border border-primary/30 overflow-hidden">
                        <div className="flex flex-col lg:flex-row items-center gap-0">
                            ...
                            <div className="flex-1 p-6 md:p-10 lg:p-14">
                                {/* Badge EXCLUSIVO */}
                                <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-primary/20 border border-primary/40 text-primary text-[9px] md:text-[10px] font-black uppercase tracking-widest mb-6">
                                ...
                                <ul className="space-y-3 mb-10">
                                    {[
                                        'Download em qualidade 4K, Full HD ou HD',
                                        'Gerenciador de downloads automático',
                                        'Controle de armazenamento local',
                                        'Disponível para todos os planos Premium',
                                    ].map((item) => (
                                        <li key={item} className="flex items-start gap-3 text-xs md:text-sm text-gray-300">
                            ...
                            <div className="flex-shrink-0 w-full lg:w-80 bg-white/[0.02] border-t lg:border-t-0 lg:border-l border-white/5 p-6 md:p-8 flex flex-col gap-4">
                                <p className="text-[10px] font-black uppercase tracking-widest text-gray-500 mb-2">Downloads Ativos</p>
                                ...
                                {[
                                    ...
                                    <div key={i} className="bg-white/5 rounded-2xl p-3 md:p-4 border border-white/5">
                                        <div className="flex items-start justify-between mb-2">
                                            <div>
                                                <p className="text-xs md:text-sm font-bold text-white leading-tight">{dl.title}</p>
                                                <p className="text-[9px] md:text-[10px] text-gray-500 mt-0.5">{dl.quality} · {dl.size}</p>
                                            </div>
                                            <div className={`w-6 h-6 md:w-7 md:h-7 rounded-full flex items-center justify-center flex-shrink-0 ${dl.progress === 100 ? 'bg-green-500/20 text-green-400' : 'bg-primary/20 text-primary'}`}>
                                                {dl.progress === 100
                                                    ? <FiCheck size={12} />
                                                    : <FiDownload size={12} className="animate-bounce" />
                                                }
                                            </div>
                                        </div>
                                        {/* Barra de progresso */}
                                        ...
                                        <p className="text-[8px] md:text-[9px] text-gray-600 mt-1.5 text-right">
                                            {dl.progress === 100 ? 'Concluído ✓' : dl.progress === 0 ? 'Na fila...' : `${dl.progress}%`}
                                        </p>
                                    </div>
                                ))}
                            </div>
                ...
                <div className="px-4 md:px-0">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
                        {features.map((feature, idx) => (
                            <div
                                key={idx}
                                className="glass-panel p-6 md:p-8 rounded-3xl group hover:scale-[1.02] transition-all duration-500 hover:border-primary/30 reveal"
                            >
                                <div className={`w-12 h-12 md:w-14 md:h-14 rounded-2xl bg-white/5 flex items-center justify-center mb-6 transition-colors group-hover:bg-primary/10 ${feature.color === 'blue' ? 'text-neon-blue' : 'text-neon-purple'}`}>
